...
from django.db.models import Q, DecimalField, DateTimeField, DateField, BooleanField, IntegerField, CharField
...
class DatatablesColumnSimpleRender(object):
    def render(self, request, model, field_name):
        ...
        if isinstance(model_field, RelatedField):
        ...
        elif isinstance(model_field, DecimalField):
            ...
            val = '是' if val == 'True' else '否'
        elif isinstance(model_field, IntegerField) or isinstance(model_field, CharField):
...
class DatatablesColumnActionsRender(object):
    ...
    def __init__(self, actions=None, action_permission=None):
        ...
        if actions is None:
            self.actions = \
                [{'is_link': True, 'name': 'edit', 'text': '编辑', 'icon': 'fa fa-edit'},
                 {'is_link': False, 'name': 'delete', 'text': '删除', 'icon': 'fa fa-trash-o'}, ]
        else:
        ...
        for action in self.actions:
            ...
            if is_link:
                if new_window and url:
                    template = '''<a href="%(url)s" target="_blank" class='btn btn-sm btn-white btn-pink'> <i class='%(icon)s'></i>%(text)s</a>'''
                else:
                    template = '''
...
<div class="btn-group">
  ...
  <ul class="pull-right dropdown-menu action-button">%s</ul>
            ...
            html = '<div class="btn-group">%s</div>'
...
class DatatablesColumnActionsRender2(object):
    ...
    def render(self, request, model, field_name):
        ...
        for (index, action) in enumerate(self.actions):
            ...
            if is_link:
                if new_window and url:
                    template = '''<a href="%(url)s" target="_blank" class='%(color)s' title='%(text)s'> <i class='ace-icon bigger-130 %(icon)s'></i></a>'''
                else:
                    template = '''
...
<i class='ace-icon bigger-130 %(icon)s'></i></a>'''
        ...
        actions_str = "".join(action_items)
        html = '<div class="action-buttons">%s</div>'
...
class DatatablesBooleanColumn(DatatablesColumnMixin, forms.ChoiceField):
    ...
    def __init__(self, choices=None, *args, **kwargs):
        kwargs['choices'] = choices or (('', '全部'), (1, '是'), (0, '否'))
        kwargs['is_sortable'] = kwargs.get('is_sortable', True)
    ...
    def build_aoColumnDefs(self):
        ...
        for name, field in self.fields.items():
            ...
            if not label and not is_sys_field:
                label = str(self.Meta.model._meta.get_field(name).verbose_name)
