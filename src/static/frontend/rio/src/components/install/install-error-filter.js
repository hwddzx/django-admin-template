  angular.module('stf.install-service')
      .filter('installError', installErrorFilter)
      .filter('installState', installStateFilter);

  function installStateFilter(gettext) {
  ...
  function installErrorFilter(gettext) {
      /*jshint maxlen:200*/
      return function(text) {
          switch (text) {
              // Our error codes.
              ...
              case "INSTALL_FAILED_UID_CHANGED":
                  return gettext("该APK暂时在本手机上无法安装.")
