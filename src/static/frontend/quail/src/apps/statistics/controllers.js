(function() {
    ...
    function StatisticsController($scope, config, $stateParams, DialogService, StatisticsService, ReleaseService) {
        var vm = this,
        ...
        _activate();
        function _activate() {
            ReleaseService.getReleases(appKey, { redmine: true }).then(function(res) {
                vm.versions = res;
            });
            var params = {start_date: _dateFormat(vm.startPopup.date), end_date: _dateFormat(vm.endPopup.date)};
            StatisticsService.getAppOverview(appKey, params).then(function(res) {
                vm.statistics = res;
            });
        }
        ...
        function getAppsProgress(selectVersionKey) {
            ...
            return StatisticsService.getAppOverview(appKey, params).then(function(data) {
                return vm.statistics = data;
            });
        }
